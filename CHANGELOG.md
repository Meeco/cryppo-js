...
- As a consequence of using `crypto.subtle` directly, this now requires browser support for the Web Crypto API. In practice this is essentially every browser in current use — it only excludes browsers a decade or more old (and Internet Explorer, which was never fully supported). See the README's Requirements section for details.
  - Chrome/Edge ≥37 (Aug 2014)
  - Firefox ≥34 (Dec 2014)
  - Safari/iOS Safari ≥11 (Sept 2017)
  - Android ≥5.0 (Nov 2014)
- On Node, `crypto.subtle` has been a stable, unflagged global since Node 19 (Oct 18, 2022) — comfortably below this package's existing `>=22.0.0` requirement, so nothing changes there.
...
- Production bundle size (this package's `dist/esm` bundled and minified, as a consumer's bundler would) is ~57% smaller (~54% smaller gzipped).
- Every cryptographic primitive is now substantially faster than the previous node-forge implementation (typically 5-150x, depending on the primitive and payload size) — see [`docs/webcrypto-vs-node-forge-benchmarks.md`](docs/webcrypto-vs-node-forge-benchmarks.md) for detailed numbers.
