...
describe('Encryption', () => {
  ...
  it('can encrypt with key using encryption artifacts', async (done) => {
    try {
      const key = `Îw0áï±OêsµCåfõ©bãë-ÒæÜ.E'Hµ®¨`; // some randomly generated bytes
      const data = '1';
  ...
  it('returns null if an empty string is passed in to encryptWithKey', async (done) => {
    try {
      const key = `øù@!L DRûÿ­ÙSAaÍÖ¡Ï9£S2îÏ`; // some randomly generated bytes
      const data = '';
      const result = await encryptWithKey(
  ...
  it('returns null if an empty string is passed in to encryptWithKeyUsingArtefacts', async (done) => {
    try {
      const key = `Îw0áï±OêsµCåfõ©bãë-ÒæÜ.E'Hµ®¨`; // some randomly generated bytes
      const data = '';
      const result = await encryptWithKeyUsingArtefacts(
