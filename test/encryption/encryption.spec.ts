...
describe('Encryption', () => {
  ...
  it('encrypts data using a provided key', async (done) => {
    try {
      const key = EncryptionKey.fromRaw(`øù@!L DRûÿ­ÙSAaÍÖ¡Ï9£S2îÏ`); // some randomly generated bytes
      const data = 'some secret data';
  ...
  it('can encrypt with key using encryption artifacts', async (done) => {
    try {
      const key = EncryptionKey.fromRaw(`Îw0áï±OêsµCåfõ©bãë-ÒæÜ.E'Hµ®¨`); // some randomly generated bytes
      const data = '1';
