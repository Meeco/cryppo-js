import { bytesToUtf8, decryptWithKey, decryptWithKeyDerivedFromString, utf8ToBytes } from '../src';
...
describe('aes-256-gcm', () => {
  it(`can successfully encrypt and decrypt with AES-GCM Encryption and latest serialization version`, async () => {
  ...
  it(`can successfully decrypt data that was not encoded with utf-8 earlier`, async () => {
  ...
  it(`can encrypt/decrypt bytes with AES-GCM Encryption and latest serialization version`, async () => {
  ...
  Object.values(CipherStrategy).forEach((strategy) => {
    Object.values(SerializationFormat).forEach((version) => {
      it(`can successfully encrypt and decrypt with ${strategy}
         Encryption and ${version} serialization version`, async () => {
